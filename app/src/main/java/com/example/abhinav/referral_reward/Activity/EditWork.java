...
public class EditWork extends AppCompatActivity {
    ...
    public void saveEditWork(final UserWorks work){
        btm = (Button) findViewById(R.id.edit_work);
        btm.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        loginApi.edit_UserWork(params, id, new JsonHttpResponseHandler(){
                            @Override
                            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                                super.onSuccess(statusCode, headers, response);
                                try {
                                    if(Integer.parseInt(response.getString("status")) == 1){
                                        work.setComments(response.getString("comments"));
